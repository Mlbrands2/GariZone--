...
export const Sidebar = ({ minMaxValues, searchParams }: SidebarProps) => {
  ...
  return (
    <aside className="w-72 min-h-screen bg-slate-50 border-r border-slate-200 p-6 hidden lg:block">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-slate-700 text-lg font-semibold tracking-tight">
          Filters
        </h3>
        <button
          type="button"
          ...
          className={cn(
            ...
            !filterCount
              ? "disable opacity-50 pointer-events-none cursor-default"
              : "hover:underline cursor-pointer"
          )}
        >
          Clear All{filterCount ? ` (${filterCount})` : ""}
        </button>
      </div>
